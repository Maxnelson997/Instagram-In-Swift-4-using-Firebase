...
class SignUpController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (user, error) in
            ...
            Storage.storage().reference().child("profile_image").child(filename).putData(uploadData, metadata: nil, completion: { (metadata, err) in
                ...
                Database.database().reference().child("users").updateChildValues(values, withCompletionBlock: { (err, ref) in
                    ...
                    print("successfuly saved user into db")
                })
    ...
    override func viewDidLoad() {
        ...
        alreadyHaveAccountButton.addTarget(self, action: #selector(self.handleAlreadyHaveAccount), for: .touchUpInside)
        alreadyHaveAccountButton.anchor(top: nil, left: view.leftAnchor, bottom: view.bottomAnchor, right: view.rightAnchor, paddingTop: 0, paddingLeft: 0, paddingBottom: 0, paddingRight: 0, width: 0, height: 50)
        
        
        view.backgroundColor = .white
        
        emailTextField.addTarget(self, action: #selector(self.handleTextInputChange), for: .editingChanged)
        usernameTextField.addTarget(self, action: #selector(self.handleTextInputChange), for: .editingChanged)
        passwordTextField.addTarget(self, action: #selector(self.handleTextInputChange), for: .editingChanged)
        plusPhotoButton.addTarget(self, action: #selector(self.handlePlusPhoto), for: .touchUpInside)
